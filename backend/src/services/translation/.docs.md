...
## Message Translation (FEATURE-018)

### Overview

FEATURE-018 extends the translation service to support **real-time message translation** for multi-user chats. Messages are automatically translated to the preferred language of each member who has enabled auto-translate.
...
model UserConversationMembership {
  ...
  autoTranslateEnabled Boolean @default(false)  // NEW: Per-user auto-translate toggle
  // ... other fields
}
```
...
// Translate a message to multiple languages at once (batch)
...
Translations are **pre-generated** when messages are sent in multi-user conversations:

```typescript
// In chatHandler.ts - when a user sends a message
const members = await prisma.userConversationMembership.findMany({
  where: {
    conversationId: payload.conversationId,
    isActive: true,
    autoTranslateEnabled: true,  // Only for members with auto-translate enabled
    userId: { not: user.id }      // Exclude sender
  }
});

const uniqueLanguages = [...new Set(members.map(m => m.user.preferredLanguage))];

// Pre-generate translations asynchronously
...
The frontend uses the `useMessageTranslations` hook to manage translations:

```typescript
const {
  getTranslatedText,      // Get translated text for a message
  hasTranslation,         // Check if translation exists
  toggleTranslation,       // Toggle between original/translated
  requestTranslation,     // Request translation on-demand
  isTranslationLoading    // Check if translation is in progress
...
// Display translated text (or original if no translation)
const displayText = isSent ? message : getTranslatedText(messageId, message);
```

### Important Notes

1. **Encrypted Messages**: Messages are encrypted in the database. The translation service decrypts them before translating.

2. **Sender Exclusion**: A user's own messages are NEVER translated for them (only for other members).

3. **Bot Messages**: When bots respond, translations are also pre-generated for members with `autoTranslateEnabled: true`.

4. **Language Detection**: If the message is already in the target language, no translation is generated.

5. **Unique Constraint**: Each `(messageId, targetLanguage)` pair is unique - duplicate translations are ignored.

### Testing Message Translation

Run the message translation tests:

```bash
cd backend
npm test -- translationService.message.test.ts
npm test -- memberships.translation.integration.test.ts
```
